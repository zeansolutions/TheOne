...
### 3. طبقة الجهة والالتزام (`modality_processor.py` / `modality.json`)
...
1. لتشغيل اختبارات التحقق الآلية لكافة الطبقات المعرفية، قم بتشغيل الأمر التالي:
   ```bash
   ./start.sh test
   ```
2. في التيرمينال التفاعلي (الخيار رقم **1**)، اطرح أسئلة مركبة تحتوي على خصائص الطبقات، على سبيل المثال:
   * **النفي:** `"هل الأسد ليس حيواناً؟"` (يكتشف النظام النفي ويرفض مقدمة السؤال).
   * **المقارنة:** `"من الأسرع، الأسد أم الدب القطبي؟"` (يفكك المحرك مقارنة السرعة ويعيد النتيجة).
   * **الاستثناء والشذوذ:** `"أسد نباتي يأكل العشب"` (يكشف النظام شذوذ الكيان ويخفض ثقة المعلومة).
